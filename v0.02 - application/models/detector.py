...
# FIX applied:  The imputer was fitted with a DataFrame so we must
#               always pass a DataFrame (not a numpy array) to transform().
#               This silences the sklearn feature-name warning completely.
# """
...
# # Suppress the one remaining sklearn internal warning
...
#         # Feature names the imputer/scaler were fitted on
...
#         """
#         df    = self._to_dataframe(features)          # always a DataFrame
#         df    = pd.DataFrame(
#                     self.imputer.transform(df),
#                     columns=self.feature_names)        # impute, keep names
#         df    = pd.DataFrame(
#                     self.scaler.transform(df),
#                     columns=self.feature_names)        # scale, keep names
#         probs = self.model.predict_proba(df.values)[0]
...
#                     self.imputer.transform(feat_df),
#                     columns=self.feature_names)
#         feat_df = pd.DataFrame(
#                     self.scaler.transform(feat_df),
#                     columns=self.feature_names)
#         probs  = self.model.predict_proba(feat_df.values)
